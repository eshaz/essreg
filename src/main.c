...
int main(int argc, char *argv[]) {
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (arg_data == NULL) {
      ...
      if (argv[i][0] == '3') {
        if (strstr(arg_data, "%"))
          set_3d_level_pct(atoi(arg_data));
        else
          set_3d_level(atoi(arg_data));
      } else if (argv[i][0] == 'm') {
        if (argv[i][1] == 'l') {
          if (strstr(arg_data, "%"))
            set_mono_in_level_pct(atoi(arg_data));
          else
            set_mono_in_level(atoi(arg_data));
        } else if (argv[i][1] == '=')
          set_mono_in(atoi(arg_data));
      } else if (argv[i][0] == 'r')
        dump_regs(arg_data);
      else if (argv[i][0] == 'p')
        set_mic_preamp(atoi(arg_data));
      else if (argv[i][0] == 'd')
        set_digital_record(atoi(arg_data));
      else if (argv[i][0] == 't')
        set_telegaming_mode(atoi(arg_data));
    }
  }
